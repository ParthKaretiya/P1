...
const HERO_WIDTHS = [480, 560, 768, 1100, 1376]
...
/* Parallax only above 900px — on phones the effect is barely visible (the
   visual column stacks below the text), but useScroll forces layout reads
   (offsetTop/scrollHeight) interleaved with Motion's transform writes on
   every scroll frame — the main "forced reflow" Lighthouse flags on mobile.
   useScroll lives in this child so the hook (and its per-frame measurement)
   only exists when parallax is actually rendered. */
...
const isDesktopNow = () => window.matchMedia(DESKTOP_MQ).matches

function HeroParallax({ target, children }) {
  ...
  return children({ yEditor, yMid, yFront })
}

/* The three layered mockup cards. Shared between the static (mobile /
   reduced-motion) branch and the HeroParallax (desktop) branch — the y
   motion values are simply undefined in the static case. */
function renderMockupLayers({ yEditor, yMid, yFront }) {
  return (
    <>
      {/* Layer 1 (back) — classroom photo card */}
      <m.div
        className={styles.photoCard}
        style={yEditor && { y: yEditor }}
      >
        <picture>
...
export default function Hero() {
  ...
  const noParallax = reduce || !isDesktop
